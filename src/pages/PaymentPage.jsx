...
//                 const options = {
//                     key: 'rzp_test_enKDPSrgUdxoK8', // Ensure this matches your backend's key_id
//                     amount,
//                     currency,
//                     name: 'E-commerce Store',
//                     description: 'Order Payment',
...
const PaymentPage = () => {
    ...
    const [orderDetails, setOrderDetails] = useState(() => {
        const stateData = location.state?.orderDetails;
        ...
        try {
            ...
            if (selectedPaymentMethod === 'cod') {
                ...
                const options = {
                    ...
                    handler: async function (razorpayResponse) {
                        try {
                            const saveResponse = await fetch('http://localhost:3001/api/orders', {
                                method: 'POST',
                                headers: { 'Content-Type': 'application/json' },
                                body: JSON.stringify({
                                    ...orderDetails,
                            ...
                            if (!saveResponse.ok) {
                                ...
                                throw new Error(err.message || "Payment was successful, but saving order failed.");
                            }
                ...
                toast.warn(`Selected payment method (${selectedPaymentMethod}) is not implemented.`);
            }
        } catch (error) {
            toast.error(`Payment failed: ${error.message}`);
        } finally {
            if (selectedPaymentMethod === 'cod') setLoading(false);
        }
    };
        ...
        <div className="min-h-screen bg-gray-100 py-10 px-4">
            ...
            <div className="max-w-xl mx-auto bg-white rounded-lg shadow-lg p-8">
                ...
                <div className="mb-8 p-6 bg-gray-50 rounded-lg border">
                    ...
                    {orderDetails.cartItems.map(item => (
                        <div key={item._id} className="flex justify-between mb-4 border-b pb-4">
                            <div className="flex space-x-4">
                                <img
                                    src={item.image?.startsWith('http') ? item.image : `http://localhost:3001/images/${item.image}`}
                    ...
                    <div className="flex justify-between pt-4 border-t mt-4 text-lg font-semibold">
                        <span>Delivery Charges:</span>
                        <span>{orderDetails.deliveryOption === 'express' ? '₹100.00' : 'Free'}</span>
                    </div>
                    <div className="flex justify-between text-xl font-bold mt-4">
                        <span>Total Payable:</span>
                        <span className="text-green-600">₹{orderDetails.grandTotal.toFixed(2)}</span>
                    </div>
                </div>
                ...
                <div className="mb-8 p-6 bg-gray-50 rounded-lg border">
                    ...
                    {selectedPaymentMethod === 'card' && (
                        <p className="mt-2 text-sm text-gray-600">
                            You will be redirected to Razorpay secure gateway to complete your transaction.
                        </p>
                    )}
                    {selectedPaymentMethod === 'cod' && (
                        <p className="mt-2 text-sm text-gray-600">
